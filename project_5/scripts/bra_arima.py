...
def create_model(series):
    model = ARIMA(series, order=(0,2,3), enforce_invertibility=False)
    results = model.fit()
...
def create_arima_plot(df, results, option):
    ...
    plt.title('Average temperature by year in Brazil\nARIMA model', size=26)
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] not in ['0', '1']:
        print("Usage: python bra_arima.py [0|1]")
        print("0 - show plot interactively, 1 - save plot to file")
        sys.exit(1)
    option = int(sys.argv[1])

    df = read_dataframe('data/brazil_yearly_avg.csv')
