...
def create_model(df):
    model = ExponentialSmoothing(df['AverageTemperature'], trend='add', seasonal=None, initialization_method='estimated')
    results = model.fit()
    return results
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] not in ['0', '1']:
        print("Usage: python new_hwes.py [0|1]")
