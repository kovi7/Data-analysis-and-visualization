...
semester_colors = {"Winter": "cornflowerblue", "Summer": "wheat", 'Vacation': "lightgreen"}
...
for i, (_, event) in enumerate(data.iterrows()):
    ...
    ax.barh(y_positions[i], duration, height=bar_height, left=mdates.date2num(start), 
            color=semester_colors.get(event["Semester"]), edgecolor='black', alpha=0.8)
    
    #labels
    bar_width = mdates.date2num(end) - mdates.date2num(start)
    if bar_width > len(label)*3: 
        pos = mdates.date2num(start) + bar_width/2
        ha = 'center'
    else:
       pos = mdates.date2num(end) + 2
       ha='left' 

    ax.text(pos, y_positions[i], 
            label, ha=ha, va='center', 
            fontweight='bold', fontsize=9)

# legend + abbreviation explanations
legend_elements = [Rectangle((0, 0), 1, 1, color=semester_colors.get(semester), label=semester) for semester in data['Semester'].unique()]
...
plt.savefig('../images/fig18.pdf', bbox_inches='tight')
plt.savefig('../images/fig18.png', bbox_inches='tight', dpi=300)
plt.close()