...
country_texture = {country: texture_types[i] for i, country in enumerate(chosen_countries['Country Name'])}
...
def animate(year):
    ...
    ax.text(x=0.09, y=0.8, s=year, fontsize=45, transform=ax.transAxes, color = "black",
            fontweight = 'bold' if 1975 <= float(year) <= 1979 else 'normal')
    ax.tick_params(axis='both', labelsize=20)
    ax.axhline(y=cambodia_pop_1975, linestyle='--', color = 'black')    

    bars = ax.bar(chosen_countries['Country Name'], chosen_countries[year].values, color='gray')
    
    # Cambodian Genocide (1975-1979)
    if  1975 <= float(year) <= 1979:
        ax.text(0.6, 0.8, 'Cambodian Genocide', transform=ax.transAxes, 
                fontsize=45, color='black', ha='center', weight = 'bold')
    ...
    for i, bar in enumerate(bars):
        ...
        add_texture(bar, texture_type=country_texture[chosen_countries['Country Name'].iloc[i]])
...
ani.save('../images/fig17.gif', writer='imagemagick')
